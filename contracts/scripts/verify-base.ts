...
/**
 * Verification script for Base deployment
 *
 * Usage:
 * 1. Deploy contracts using: npx hardhat run scripts/deploy-base.ts --network base
 * 2. Copy deployed addresses below
 * 3. Run: npx hardhat run scripts/verify-base.ts --network base
 */

async function main() {
  console.log("🔍 Verifying Liquium contracts on Base...\n");
  ...
  try {
    ...
    // 4. Verify MockPriceReader (for Base - no FTSO)
    console.log("4️⃣  Verifying MockPriceReader...");
    await verifyContract(
      {
    ...
    console.log("✅ MockPriceReader verified!\n");
    ...
    console.log(`  MockPriceReader:          ${addresses.PRICE_READER}`);
    ...
    console.log(`🔗 View on ${hre.network.name === "baseSepolia" ? "Base Sepolia" : "Base"} Explorer:`);
    console.log(`${explorerUrl}/address/${addresses.DEAL_VAULT}#code\n`);

    console.log("✨ Next Steps:");
    console.log("1. ✅ Contracts deployed and verified");
    console.log("2. Fund MockProtocol with test tokens");
    console.log("3. Create your first deal");
